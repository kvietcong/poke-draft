...
export const GameTradesAccordion = () => {
    ...
    return (
        <Accordion>
            {gameTrades.length > 0 ? gameTrades.map((trade) => {
                ...
                const TradeTitle = (
                    <Text>
                        ...
                        <span
                            style={{ float: "right", paddingRight: "0.5rem" }}
                        >
                            {trade.id}
                        </span>
                    </Text>
                ...
                return (
                    <Accordion.Item key={trade.id} value={trade.id}>
                        <Accordion.Control>{TradeTitle}</Accordion.Control>
                        <Accordion.Panel>
                            <Stack align="center">
                                ...
                                {session &&
                                    [
                                        ...
                                        <Button onClick={removeTradeOrError}>
                                            Reject Trade
                                        </Button>
                                    )}
                                <Flex
                                ...
                                >
                                    {trade.transactions.map((transaction) => (
                                        <TradeLine
                                            key={
                                                transaction.selection.pokemonID
                                            }
                                            from={transaction.oldOwner.name}
                                            to={transaction.newOwner.name}
